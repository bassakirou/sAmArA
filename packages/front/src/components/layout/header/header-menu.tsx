    ...
    <nav className={classNames(`headerMenu flex w-full relative`, className)}>
      {data?.map((item: any) => (
        ...
        >
          <Link
            href={item?.path}
            className="relative inline-flex items-center text-heading px-3 py-2 text-sm font-normal before:rtl:right-0 before:ltr:left-0 xl:text-base text-heading xl:px-4 group-hover:text-black"
          >
            {t(item.label)}
