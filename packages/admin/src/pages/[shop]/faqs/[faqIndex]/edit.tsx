...
export default function ShopFaqEditPage() {
  const { t } = useTranslation(['common', 'form']);
  const router = useRouter();
  const { shop, faqIndex } = router.query;
  const index = Number(faqIndex);
  ...
  const currentFaq = Number.isFinite(index) ? faqs[index] : undefined;
  ...
  useEffect(() => {
    ...
    reset({
      ...
      order: Number(currentFaq.order ?? index),
    });
  }, [currentFaq, index, reset]);
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    const nextFaqs = faqs.map((item, idx) =>
      idx === index
        ? {
    ...
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        <div className="my-5 flex flex-wrap sm:my-8">
          <Description
            title={t('common:sidebar-nav-item-faqs')}
...
ShopFaqEditPage.authenticate = {
  permissions: adminOwnerAndStaffOnly,
};
ShopFaqEditPage.Layout = ShopLayout;
