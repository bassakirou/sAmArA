...
import { adminAndOwnerOnly, getAuthCredentials, isSuperAdmin } from '@/utils/auth-utils';
...
export default function NotificationsPage() {
  ...
  const items = useMemo(() => {
    return (storeNotices ?? []).map((notice: any) => {
      ...
      if (kind === 'chat_message' && data?.conversation_id) {
        ...
        href = Routes.order.details(String(data.order_id));
  ...
  }, [isAdminUser, myShops, storeNotices]);
...
NotificationsPage.authenticate = {
  permissions: adminAndOwnerOnly,
};
NotificationsPage.Layout = NotificationsLayout;
