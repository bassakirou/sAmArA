...
export default function GuidelineEditor({ control, register }: any) {
  const name = 'sections.guideline';
  const items = useFieldArray({ control, name: `${name}.data.items` as any });
  return (
    <PageSectionEditor
      title="Guidelines"
      details="Regles ou conseils a suivre pour vendre sur la plateforme."
      name={name}
      control={control}
      register={register}
    >
      ...
      <Button type="button" onClick={() => items.append({ title: '', link: '' })}>
